...
docker-compose down
docker ps -a --format '{{.Names}}' | grep -E '(^|_)ai_(backend|frontend|redis)_1$|ai_(backend|frontend|redis)_1|backend|frontend|redis' | xargs -r docker rm -f 2>/dev/null || true
